...
// returns distance in CM (in float)
uint16_t measure_distance() {

  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);
  timer_count = 0;
  HAL_UART_Transmit(&huart2, (const uint8_t*)"A", 1, 100);
  while(timer_count < 10);
  // HAL_UART_Transmit(&huart2, (const uint8_t*)"B", 1, 100);
  ...
  HAL_UART_Transmit(&huart2, (const uint8_t*)"D", 1, 100);
  // return (float)timer_count / 58;
  return timer_count;
...
int main(void)
{
  ...
  HAL_UART_Transmit(&huart2, (const uint8_t*)"hello 1", 7, 100);
  MX_TIM2_Init();
  HAL_UART_Transmit(&huart2, (const uint8_t*)"hello 2", 7, 100);
  /* USER CODE BEGIN 2 */
  ...
  while (1)
  {
    ...
    sprintf(text, "dist=%d\r\n", distance);
    HAL_UART_Transmit(&huart2, (const uint8_t*)text, 15, 100);
